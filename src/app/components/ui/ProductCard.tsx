...
export default function ProductCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex flex-col w-[290px] shrink-0 rounded-2xl border overflow-hidden backdrop-blur-md transition-all duration-500 shadow-lg",
        isAgent
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-10 flex flex-wrap gap-1">
        <span className={cn(
          "px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase shadow-sm",
          isAgent
        ...
        {product.tags.slice(0, 1).map((tag, idx) => (
          <span key={idx} className={cn(
            "px-2.5 py-0.5 rounded-full text-[10px] font-medium backdrop-blur-md shadow-sm border",
            isAgent
      ...
      {/* Product Image Area */}
      <div className={cn(
        "relative w-full h-[160px] flex items-center justify-center overflow-hidden border-b",
        isAgent ? "bg-slate-50/50 border-slate-100" : "bg-black/20 border-white/[0.05]"
      )}>
        {/* Glow behind image on hover */}
        <div className={cn(
          "absolute w-36 h-36 rounded-full blur-3xl opacity-20 -z-10 transition-transform duration-500 scale-100 group-hover:scale-125",
        ...
        <motion.img
          ...
          src={product.image}
          alt={product.name}
          className="w-[120px] h-[120px] object-contain drop-shadow-[0_8px_16px_rgba(0,0,0,0.15)] filter"
        />

        {/* Color selectors overlay */}
        <div className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-black/30 backdrop-blur-md px-2 py-1 rounded-full border border-white/10 z-10">
          {product.colors.map((color, idx) => (
            <button
              key={idx}
              onClick={() => setSelectedColor(idx)}
              className={cn(
                "w-3.5 h-3.5 rounded-full border border-white/40 transition-all duration-200 hover:scale-110",
                selectedColor === idx ? "ring-2 ring-white scale-105" : "opacity-75"
              )}
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          ...
          {/* Product Name */}
          <h3 className={cn(
            "text-base font-bold tracking-tight line-clamp-1 mb-1.5",
          ...
          {/* Price */}
          <div className={cn(
            "text-lg font-extrabold tracking-tight mb-3.5",
          ...
          <div className={cn(
            "grid grid-cols-2 gap-2 text-[11px] font-medium py-3 border-y mb-4",
            ...
            <div className="flex items-center gap-1.5">
              <Battery className="w-3.5 h-3.5 shrink-0 text-slate-400" />
        ...
        <div className="flex gap-2 shrink-0">
          <button
            ...
            className={cn(
              "flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold border transition-all duration-300 bg-white border-slate-200 text-slate-700",
              isAgent
          ...
          <button
            ...
            className={cn(
              "flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold text-white transition-all duration-300 shadow-md",
              isAgent
